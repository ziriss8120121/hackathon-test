...
推論を呼ばずに受入基準の大半を検証できるようにするため、テストは ScriptedBrain
だけを使う。LLMを呼ばないので、無料枠もモデルのダウンロードも消費しない（CIでも
同じ構成で動く）。
"""
...
@pytest.fixture
def v2_config():
    def build(**overrides: Any) -> config_module.ExperimentConfig:
        overrides.setdefault("machine_name", "test")
        return config_module.load_config(overrides=overrides)
