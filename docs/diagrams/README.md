...
設計書v2.1の図に対応します。図の順序は `docs/design.md` に現れるmermaidブロックの順序です。

図の数と順序はv2.0から変わっていませんが、`03-case-flow` と `09-game-phases` は内容が変わっています（ルール文書v0.6の反映）。PNGを生成する際は全11枚を作り直してください。
