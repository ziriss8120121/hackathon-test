...
`brains/base.py` に置く。ゲーム進行側とJudgeが知るのはこの形だけである。v1.1から変えたのは `Request.subjects` の1項目だけである（下の注記）。
...
class Request:
    ...
    subjects: tuple[str, ...]     # 1回の応答で1件ずつ答える対象。Judgeの発言バッチで使う
...
`subjects` はv2.0で足した。エージェントの呼び出しは1回に1つの判断を返すが、Judgeは発言バッチの全件へ1回で答える（5.5）。どの発言に答えるべきかを渡す場所がないと、`speech_id` を本文の中だけに置くことになり、Stubがバッチに対応する応答を作れない。`choices` で兼ねさせない理由は、`choices` が「1つの項目に入る値の候補」であるのに対し、`subjects` は「答えを1件ずつ返す対象」だからである。Judgeでは前者が参加者ID、後者が `speech_id` になり、混ぜると形式の再指定（5.4）で両方が「選べる値」として提示される。
...
対象が空のスタンスと、発言者自身へ向いたスタンスは記録せずに落とす。どちらも「各人の最新スタンスを1件ずつ保持する」正規化（下の導出手順）に使えず、疑念分布の対象にならないためである。対象が空のスタンスを残すと、`stances` が空の発言と分布への寄与が同じでありながら形だけ違う2種類の記録ができる。

**評価できなかった発言の扱い**

Judgeの応答は、バッチへ渡した `speech_id` と1対1に対応していなければならない。対応が取れない場合、そのバッチを最大3回まで送り直す。送り直しの回数はJudge側の定数であり、ルール文書v0.7 §1 の回答機会（4.1）とは無関係である。Judgeはゲームの参加者ではないため、ルールが定める回答機会の対象にならない。

途中の応答で対応が取れた発言は保持し、送り直しでは欠けている発言だけを見る。上限まで対応が取れなかった発言は、評価を作らずに `parse_failed: true` で残す（6.8）。ここで `other` などの既定値を入れると、評価できなかったことと「どれにも当てはまらない」ことが区別できなくなり、9章のラベル分布が静かに歪む。

`speech_id` が対象のバッチに含まれない評価は捨てる。小型モデルがIDを作り出した場合に、存在しない発言の評価が混ざらないようにするためである（AC-06）。
...
3. スタンスを含まない発言では、その発言者の直前のスタンスをそのまま残す。
4. その時点のテーブルから疑念分布を作る。`direction` が `suspect` のスタンスについて、対象ごとに人数を数える。1人1件なので、分布の合計は最大8になる。
5. 各発言時点の分布を `stance_series` として保存する。

この処理により、発言回数の多いプレイヤーの疑いが分布へ重複して数えられない（F-44）。分布はJudgeの出力から機械的に導出するので、Judgeを呼び直さずに算出方法を変えられる。

手順3を「保持」にしたのは、スタンスが「現在の立場」という状態だからである。質問や自分の役職主張のように立場を示さない発言をしたことは、直前に示した立場の取り下げにはあたらない。`null` に戻す形にすると、立場を変えていない人が発言するたびに分布から出入りし、`final_entropy`（9.2）が会話の収束ではなく発言の種類に反応する。

疑いが1件もない時点のエントロピーは0とする。分散しているのではなく分布が存在しないため、「完全な分散」を表す1にはしない。9.2の `final_entropy` は最終発言時点の値なので、議論が進めば通常この状態にはならない。
...
`speeches` は `case_log.json` の発言と同じ順で、同じ件数だけ並ぶ。見送りとスキップには `speech_id` が振られていないため対象外になる（6.7）。`parse_failed: true` の項目は「評価できなかった発言」であり、`labels` は空になる。`other` は入れない（5.5）。分析側はこの2つを区別し、`parse_failed` の件数をレポートへ出す。

`stance_series` の `current_stances` は参加者8人分のキーを必ず持つ。まだ立場を示していない人は `null` になる。キーを間引くと、立場を示していないことと評価が欠けていることが区別できない。
...
上位のHTML（Trial・実験・RQ）は集計値だけなので埋め込みでよい。並べ替えや絞り込みを画面上で行いたい対象であり、そのときJSONが手元にある形が扱いやすい。同じ内容はPython側でも表として書き出す。JSONだけだと、JavaScriptが動かない環境で空白のページになるためである。
...
| Judgeだけ別の脳で評価する | `python -m mbti_werewolf judge --experiment e-... --judge-brain gemini --judge-model gemini-2.0-flash` |
...
| Judge評価（`judge.v1.json`）がない | そのTrialを除外する。17ケースが揃っていても、疑念分布が無い状態で収束の指標を比較すると空欄と値が混ざる |
...
Wilcoxon符号付順位検定を選んだ理由は3つある。1つは指標が比率や順序尺度を含み、正規分布を仮定できないこと。2つは17ケースを1組とする対応あり構造をそのまま使えること。3つは順位計算だけで実装できるため、SciPyを追加せずに済むこと（1.1）。組数が20以下のときは符号の全列挙で正確なpを出し、それを超えたら正規近似にする。
...
| `test_judge` | Judgeが発言単位の評価を返し、`speech_id` と1対1に対応する。見送りとスキップは対象外になる。存在しない `speech_id` の評価を捨て、欠けた発言を送り直し、上限まで取れなければ `parse_failed` で残す。9種にないラベル、対象が候補外・発言者自身のスタンス、不正な向きと強度を落とす。役職・MBTI・投票・勝敗・memoがプロンプトに出現しない。バッチはその最後の発言までの会話しか見ない。評価基準版を変えると別ファイルになり、評価済みのケースは `--force` なしでは対象にならない | F-40、F-42、F-45、F-46、AC-06 |
