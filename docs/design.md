...
memoは9章の指標の入力にしない。指標の計算に使わず、`transcript.md`・集計CSV・Judgeの入力にも含めない。人が1ケースを読んで挙動を説明するための材料として持つ。ただしmemoを取ること自体が応答形式とプロンプトを変えるため、実験条件の一部として扱い、`persona_prompt_version` で識別する。memoの有無が異なる結果を混ぜて分析しない。
