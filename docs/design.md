...
| 区分 | 採用 | 料金・ライセンス | 選定理由 |
| --- | --- | --- | --- |
| 言語 | Python 3.12以上 | PSF License（無料） | 標準ライブラリだけで乱数のseed管理、JSON、CSV、日時、HTTPサーバーが揃う。 |
...
| 推論（既定） | Ollama + `gemma3:4b` | Ollama本体はMIT。モデルの重みは無償利用可。 | ローカル完結で回数制限がない。100試合以上の実行はこの経路しか成立しない。 |
...
依存パッケージは `playgrounds/mbti-werewolf/requirements.txt` に固定する。バージョンは実装時に実際にインストールした値を記録する（本書では推測値を書かない）。
...
| 項目 | 決める時期 |
| --- | --- |
| 依存パッケージの固定バージョン | 実装時に実際にインストールした値を `requirements.txt` に記録する |
| 使用するモデルの最終選定と利用規約の確認記録 | M2で実機確認したうえで `docs/` の実行メモに残す |
| プロンプトv1の具体的な文面 | M2で作り、`agents/prompts/v1/` に置く |
| 指標判定の語彙リスト | M1で `record/metrics.py` に定数として置く |
| 性能の閾値 | 初回実測後に要求定義書へ反映する |
