...
| 区分 | 指標 | 現状値 | 目標 |
| --- | --- | --- | --- |
| 主要KPI | 4人MVPの実行完了 | stub および ollama/`gemma3:4b` で完了（2026-08-15） | 1ゲームを自動実行し、勝敗・投票・ログが残る。 |
...
| ゆうじろう | Engineer | M2 MacBook Air | Cursor利用。ChatGPT/Codex契約状況は要確認。 | API課金は使わない前提。Ollama 0.32.13 + `gemma3:4b` で1試合約2分50秒を確認（2026-08-15）。 | 100試合換算は約4.7時間。夜間実行で成立する。 |
