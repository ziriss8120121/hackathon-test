...
| --- | --- | --- | --- |
| A1 比較単位 | 現行の`series`は、seedを変えた独立したrunの集合であり、役職などの実験条件もrunごとに変わる。 | MixedとHomogeneousの差に役割など他条件の差が混ざり、MBTI構成だけを比較できない。 | 同じ人物、年齢、性別、役割、ゲーム・議論条件を共有し、MBTI構成だけが異なる17ケースを1 Trialとして識別・実行・集計できる。 |
...
flowchart TD
    ...
    B --> C["Mixed 1ケースとHomogeneous 16ケースを生成\n変更するのはMBTI構成のみ"]
